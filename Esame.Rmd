...
Lo scopo di questo progetto è di analizzare alcuni aspetti connessi al fenomeno del terrorismo, che ha come obbiettivo quello di incutere terrore nei membri di una società e/o di influenzare scelte politiche di un paese/organizzaione.
...
```{r iibraries, message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE}
library(readr)
library(tidyr)
library(dplyr)
...
Di seguito è visibile, che il numero di attacchi terroristici è cresciuto notevolmente dopo l'anno 2011, raggiungendo quasi 17000 attacchi all'anno durante il 2014. Nel 2014, infatti, si sono registrati più del triplo degli attacchi registrati nel 2011. Cerchiamo di capirne il motivo andando ad isolare i gruppi terrostici più attivi tra il 2012 ed il 2017.
...
Dal grafico notiamo, che i 5 gruppi terroristici più attivi per ciascun anno in analisi, sono quasi per la totalità gruppi riconducubuli al terrorismo di matrice islamica. Notiamo, che dal 2013 è cresciuto notevolmente il numero di attacchi perpetrati dall'ISIL (Islamic State of Iraq and the Levant), gruppo molto attivo nel reclutamento di proseliti al fine di commettere attacchi terrostici all'estero e/o al fine di ingrandire le proprie milizie disclocate in Siria ed Iraq (c.d. Foreign Fighters). Va riportato, inoltre, che il 29 giugno 2014, venne proclamato il califfato dello Stato Islamico, dando ancora più credito e forza nel reclutamento di nuovi militanti. 
...
```{r mapIT, echo=FALSE, message=FALSE, warning=FALSE, include=TRUE, paged.print=FALSE}
...
Notiamo, che la stragrande maggioranza dei gruppi più prolifici è da ricondurre ai gruppi di estrema sinistra. Il Baader-Meinhof Group era un gruppo di estrema sinistra operante in Germania, con affinità politiche con le Brigate Rosse. 
